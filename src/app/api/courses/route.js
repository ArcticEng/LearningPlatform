...
export async function POST(req) {
  ...
  const { title, description, price, maxEnrollment, promoLabel, isBundle, bundleCourseIds } = await req.json();
  if (!title) return NextResponse.json({ error: "Title required" }, { status: 400 });

  const course = await prisma.course.create({
    data: { title, description: description || "", price: price || 0, maxEnrollment: maxEnrollment || 0, promoLabel: promoLabel || "", isBundle: isBundle || false, bundleCourseIds: bundleCourseIds || "", tenantId: user.tenantId },
  });
...
export async function PUT(req) {
  ...
  const { id, title, description, price, maxEnrollment, promoLabel, isBundle, bundleCourseIds } = await req.json();
  ...
  if (isBundle !== undefined) updateData.isBundle = isBundle || false;
  if (bundleCourseIds !== undefined) updateData.bundleCourseIds = bundleCourseIds || "";
